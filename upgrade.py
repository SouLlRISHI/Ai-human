...
MAX_AI_WORKERS = 5
MAX_FILE_WORKERS = 10
MAX_URL_WORKERS = 100  # For concurrent path discovery
OUTPUT_DIR = "scan_results"
LOG_FILE = "vuln_scan.log"
HIDDEN_PATHS_FILE = "hidden_paths.txt"  # For storing discovered paths
...
class HiddenPathFinder:
    """Handles AI-powered hidden path discovery"""
    def __init__(self, model: str = DEFAULT_MODEL):
        self.model = model
        self.semaphore = asyncio.Semaphore(MAX_URL_WORKERS)
        self.timeout = aiohttp.ClientTimeout(total=3)
    
    async def generate_wordlist(self, domain: str) -> List[str]:
        """Generate context-aware wordlist using AI"""
        prompt = f"""Generate 50 most likely hidden web paths for {domain} including:
        ...
        try:
            ...
            logger.warning(f"AI wordlist generation failed, using fallback: {str(e)}")
            return ["admin", "api", "config", "backup", "wp-admin", "console"]  # Fallback

    async def discover(self, base_url: str) -> List[str]:
        """Find hidden paths with concurrent checks"""
        wordlist = await self.generate_wordlist(base_url)
        found_paths = []

        async with aiohttp.ClientSession(timeout=self.timeout) as session:
            tasks = [self._check_path(session, f"{base_url.rstrip('/')}/{path.lstrip('/')}") 
                    for path in wordlist]
            results = await asyncio.gather(*tasks)
            found_paths = [url for url, exists in results if exists]

        # Save results
        async with aiofiles.open(HIDDEN_PATHS_FILE, 'w') as f:
            await f.write('\n'.join(found_paths))
        
        return found_paths

    async def _check_path(self, session: aiohttp.ClientSession, url: str) -> Tuple[str, bool]:
        ...
        async with self.semaphore:
            try:
                ...
                logger.debug(f"Path check failed for {url}: {str(e)}")
                return url, False
...
class VulnerabilityScanner:
    def __init__(self, model: str = DEFAULT_MODEL, simulate_exploits: bool = False):
        ...
        self.path_finder = HiddenPathFinder(model)  # Integrated path finder
        self.simulate_exploits = simulate_exploits
        self.scan_stats = {
            "files_scanned": 0,
            "urls_scanned": 0,  # Track URL scans
            "hidden_paths_found": 0,  # Track discovered paths
            "files_skipped": 0,
        ...
        self.skipped_extensions = {".min.js", ".bundle.js"}
    ...
    async def scan_urls(self, urls: List[str], discover_hidden: bool = False) -> Dict[str, List[Dict]]:
        """Enhanced URL scanning with hidden path discovery"""
        results = {}
        
        if discover_hidden:
            console.print("[cyan]Starting hidden path discovery...[/]")
            discovered_paths = []
            for url in urls:
                paths = await self.path_finder.discover(url)
                discovered_paths.extend(paths)
            
            self.scan_stats["hidden_paths_found"] = len(discovered_paths)
            console.print(f"[green]Discovered {len(discovered_paths)} hidden paths![/]")
            urls = discovered_paths  # Scan discovered paths instead

        self.scan_stats["urls_scanned"] = len(urls)
        
        # Original URL scanning logic
        async with aiohttp.ClientSession() as session:
            tasks = [self._scan_single_url(session, url) for url in urls]
            url_results = await asyncio.gather(*tasks)
        
        for url, vulns in url_results:
            if vulns:
                results[url] = vulns
        
        return results

    async def _scan_single_url(self, session: aiohttp.ClientSession, url: str) -> Tuple[str, Optional[List[Dict]]]:
        """Scan a single URL"""
        try:
            async with session.get(url) as response:
                if response.status == 200:
                    content = await response.text()
                    return url, await self.scan_content(content, url)
        except Exception as e:
            logger.error(f"URL scan failed for {url}: {str(e)}")
        return url, None

    async def scan_content(self, content: str, source: str) -> List[Dict]:
        """Scan content from any source (file or URL)"""
        results = []
        lines_scanned = set()
        ...
        if results:
            ...
            if self.simulate_exploits:
                for vuln in results:
                    if vuln["severity"] in ["critical", "high"]:
                        vuln["simulation"] = await ExploitSimulator.simulate(vuln, source)
                        self.scan_stats["exploits_simulated"] += 1
                        if vuln["simulation"]["success"]:
                            self.scan_stats["exploits_successful"] += 1
                        if vuln["simulation"].get("verified", False):
    ...
    def print_results(self, report: Dict):
        ...
        if report["stats"]["hidden_paths_found"] > 0:
            summary.add_row("Hidden Paths Found", str(report["stats"]["hidden_paths_found"]))

        console.print(summary)

        # Detailed Findings
        for source, vulns in report["scan_results"].items():
            console.print(f"\n[bold underline]Source: {source}[/]")
...
async def main():
    parser = argparse.ArgumentParser(
        description="AI-Powered Vulnerability Scanner with Hidden Path Discovery",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    parser.add_argument("target", help="File, directory or URL to scan")
    ...
    parser.add_argument("--discover", "-d", help="Enable hidden path discovery (for URLs)", action="store_true")
    ...
    try:
        ...
        if os.path.isdir(args.target):
            ...
            console.print(f"[cyan]Scanning URL: {args.target}[/]")
            results = await scanner.scan_urls([args.target], discover_hidden=args.discover)
        else:
        ...
        if results:
            ...
            # Show discovered paths if found
            if args.discover and scanner.scan_stats["hidden_paths_found"] > 0:
                console.print("\n[bold]Discovered Hidden Paths:[/]")
